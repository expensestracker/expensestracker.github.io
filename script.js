...
// --- Info Modals ---
const infoContent = { 'about-link': { title: 'About Us', content: 'Track materials and project costs with ease.' }, 'privacy-link': { title: 'Privacy Policy', content: 'Your data is secure with Firebase.' }, 'contact-link': { title: 'Contact Us', content: 'Email: support@expensetracker.app' } };
...
function updateSummaries(expenses) { 
    ...
    finalExpensesEl.textContent = `₹${total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; 
    const materialTotals = expenses.reduce((acc, exp) => { const key = exp.material.trim().toLowerCase(); acc[key] = (acc[key] || 0) + exp.cost; return acc; }, {}); 
    const sorted = Object.keys(materialTotals).sort((a, b) => materialTotals[b] - materialTotals[a]); 
    if (sorted.length === 0) { materialSummaryEl.innerHTML = `<p class="text-gray-500">No expenses to summarize.</p>`; return; } 
    materialSummaryEl.innerHTML = sorted.map(key => { const total = materialTotals[key]; const displayName = key.charAt(0).toUpperCase() + key.slice(1); return `<div class="flex justify-between items-center text-sm"><span class="font-medium">${escapeHTML(displayName)}</span><span>₹${total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>`; }).join(''); 
}
