...
function setupSummaryToggle(cardId) {
    ...
    toggle?.addEventListener('click', () => {
        ...
        if (wrapper.style.maxHeight === '2000px') {
            wrapper.style.maxHeight = '190px';
            wrapper.classList.add('overflow-y-auto');
            chevron.classList.remove('rotate-180');
            setTimeout(() => wrapper.scrollTop = 0, 300);
        } else {
...
setupSummaryToggle('item-summary');
setupSummaryToggle('split-summary');
  ...
  const routeUser = async () => {
    if (user) {
      ...
      if (projectsUnsubscribe) {
          await supabase.removeChannel(projectsUnsubscribe);
          projectsUnsubscribe = null;
      }
      if (expensesUnsubscribe) {
          await supabase.removeChannel(expensesUnsubscribe);
    ...
    input?.addEventListener('input', (e) => {
        ...
        timeout = setTimeout(async () => {
            ...
            if (data && data.length > 0) {
                dropdown.innerHTML = data.map(d => `<li class="px-4 py-3 border-b border-slate-100 last:border-b-0 hover:bg-slate-50 cursor-pointer text-[16px] text-ios-label">${escapeHTML(d.material)}</li>`).join('');
...
function setupUIForUser(user) {
  ...
  if (userProfileMobile) userProfileMobile.innerHTML = `<div class="flex items-center"><div class="w-12 h-12 rounded-full border-2 border-indigo-100 overflow-hidden mr-3"><img src="${photo}" alt="User photo" class="w-full h-full object-cover"></div><div><p class="font-bold text-ios-label">${escapeHTML(user.user_metadata?.full_name || 'User')}</p><p class="text-sm text-slate-600 truncate">${escapeHTML(user.email)}</p></div></div>`;
...
const applyFilters = async (animate = false) => {
    ...
    // Fetch Aggregations via Server RPC
    ...
    if (summaryRes.error) {
        console.error("Summary RPC failed:", summaryRes.error);
    }
    ...
    renderSummaries(summaryRes.data, allExpensesForProject, animate);
};
...
async function listenForProjects(uid) {
  ...
  projectsUnsubscribe = supabase.channel(`projects_${uid}_${Date.now()}`)
...
async function updateActiveProject() {
    ...
    // Explicitly animate on first load
    await listenForExpenses(activeProjectId, true);
}
    ...
    sidebarProjectList.innerHTML = projects.map((p, index) => {
        ...
        return `
            <div class="flex justify-between items-center group rounded-xl hover:bg-slate-50 transition-colors">
                <a href="#" data-project-id="${p.id}" class="sidebar-project-link block py-2.5 px-3 text-[16px] text-ios-label truncate flex-grow">${escapeHTML(p.name)}</a>
                ${buttonsHTML}
            </div>`;
    }).join('');
...
function updateSidebarSelection() {
    document.querySelectorAll('.sidebar-project-link').forEach(link => {
        if(link.dataset.projectId === activeProjectId) {
            link.classList.add('text-indigo-600', 'bg-indigo-50', 'font-bold');
            link.classList.remove('text-ios-label');
        } else {
            link.classList.remove('text-indigo-600', 'bg-indigo-50', 'font-bold');
...
async function listenForExpenses(projectId, firstLoad = false) {
    if (expensesUnsubscribe) {
        await supabase.removeChannel(expensesUnsubscribe);
        expensesUnsubscribe = null;
    }
    ...
    await applyFilters(firstLoad);
    
    expensesUnsubscribe = supabase.channel(`expenses_${projectId}_${Date.now()}`)
...
function renderExpenses(expenses, animate = false) {
    ...
    if (expenses.length === 0) {
        expenseList.innerHTML = `<div class="text-center py-6 text-slate-500 text-sm">No transactions found.</div>`;
        return;
    }
    ...
    expenseList.innerHTML = expenses.map((expense, index) => {
        ...
        const costSign = '';
        const costColor = isIncome ? 'text-ios-green' : 'text-ios-label';
        ...
        if (expense.info) {
            ...
            if (matches.length > 0) {
                ...
                const splitsHtml = `<div class="mt-2 flex flex-wrap gap-1.5 w-full">` + 
                    matches.map(m => `
                        <div class="bg-slate-100 px-2 py-1 rounded-lg inline-flex items-center text-xs border border-slate-200">
                            <span class="text-slate-700 mr-1.5">${escapeHTML(m.name)}</span>
                            <span class="${splitCostColor} font-bold">₹${m.amount.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</span>
                        </div>
                    `).join('') + `</div>`;
                
                if (cleanInfo) {
                    cleanInfo = `<p class="text-xs text-slate-500 mt-1 truncate max-w-[140px]">${escapeHTML(cleanInfo)}</p>` + splitsHtml;
                } else {
                    cleanInfo = splitsHtml;
                }
            } else {
                cleanInfo = `<p class="text-xs text-slate-500 mt-1 truncate max-w-[140px]">${escapeHTML(cleanInfo)}</p>`;
            }
        }
            ...
            <div class="relative bg-white p-4 rounded-2xl shadow-sm flex items-center justify-between border border-slate-100 group transition-transform duration-200 swipe-front z-10 w-full touch-pan-y">
                <div class="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div class="overflow-hidden flex-1 pointer-events-none">
                        <h4 class="font-bold text-[17px] text-ios-label leading-tight truncate">${escapeHTML(expense.material)}</h4>
                ...
                <div class="text-right shrink-0 ml-3 pointer-events-none">
                    <p class="font-bold text-[17px] ${costColor}">${costSign}₹${Math.abs(expense.cost).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</p>
                </div>
            </div>
        </div>
    `}).join('');
    ...
    document.querySelectorAll('.swipe-front').forEach(el => {
        let startX = 0; let startY = 0; let currentX = 0; let isSwiping = false; let isScrolling = false;

        el.addEventListener('touchstart', e => {
            startX = e.touches[0].clientX; startY = e.touches[0].clientY;
            el.style.transition = 'none'; isSwiping = true; isScrolling = false;
        }, { passive: true });
        ...
        el.addEventListener('touchmove', e => {
            ...
            if (!isScrolling && Math.abs(deltaY) > Math.abs(deltaX) && Math.abs(deltaY) > 5) {
                isScrolling = true; isSwiping = false; el.style.transform = `translateX(0px)`; return;
            }
            ...
            if (currentX < -75) currentX = -75 + (currentX + 75) * 0.25;
            if (Math.abs(currentX) > 5) el.style.transform = `translateX(${currentX}px)`;
        }, { passive: true });
        ...
        const handleTouchEnd = () => {
            ...
            if (currentX > threshold) {
                ...
                setTimeout(() => { el.style.transform = `translateX(0px)`; handleEdit({ currentTarget: { dataset: { id: expenseId } } }); }, 150);
            } else if (currentX < -threshold) {
                el.style.transform = `translateX(-100px)`;
                setTimeout(() => { el.style.transform = `translateX(0px)`; handleDelete({ currentTarget: { dataset: { id: expenseId } } }); }, 150);
            } else {
                el.style.transform = `translateX(0px)`;
            }
            currentX = 0;
        };
        el.addEventListener('touchend', handleTouchEnd); el.addEventListener('touchcancel', handleTouchEnd);
    });
}
...
const infoContent = {
    ...
    'contact-link': {
        title: 'Contact Us',
        content: `<p class="mb-4">We're here to help! Whether you have a question about a feature, need technical support, or want to provide feedback.</p><div class="bg-slate-50 p-4 rounded-xl border border-slate-100"><h4 class="font-bold text-ios-label mb-2">Email Support</h4><a href="mailto:support@fintrack.app" class="text-indigo-600 font-bold hover:underline">support@expensetracker.app</a><p class="text-[10px] uppercase text-slate-500 mt-2">We aim to respond within 24-48 hours.</p></div>`
    }
};
...
// --- Summary Renderer ---
function renderSummaries(data, rawExpenses, animate = false) {
    if (!finalExpensesEl) return;
    
    if (!data) data = { net_balance: 0, total_income: 0, total_expense: 0, material_totals: [], split_totals: [] };
    ...
    // Today's Totals Calculator (Added dynamically left of Chevron)
    const todayStr = formatDate(new Date());
    let todayExp = 0;
    let todayInc = 0;
    rawExpenses.forEach(exp => {
        if (exp.date === todayStr) {
            if (exp.type === 'expense') todayExp += exp.cost;
            else todayInc += exp.cost;
        }
    });

    const todayContainer = document.getElementById('summary-today-totals');
    if (todayContainer) {
        if (todayExp === 0 && todayInc === 0) {
            todayContainer.innerHTML = '';
        } else {
            let html = `<div class="flex gap-2.5 items-center mr-1 text-[15px]">`;
            if (todayExp > 0) html += `<span class="text-ios-red font-bold">₹${todayExp.toLocaleString('en-IN')}</span>`;
            if (todayInc > 0) html += `<span class="text-ios-green font-bold">₹${todayInc.toLocaleString('en-IN')}</span>`;
            html += `</div>`;
            todayContainer.innerHTML = html;
        }
    }

    renderSummaryList('item-summary', data.material_totals || [], 'material_name', animate);

    const splitTotals = data.split_totals || [];
    if (splitTotals.length === 0) {
        document.getElementById('split-summary-card')?.classList.add('hidden');
    } else {
        document.getElementById('split-summary-card')?.classList.remove('hidden');
        renderSummaryList('split-summary', splitTotals, 'name', animate);
    }
}

function renderSummaryList(prefix, items, nameKey, animate) {
    const listEl = document.getElementById(`${prefix}-list`);
    const chevronBox = document.getElementById(`${prefix}-chevron-box`);
    ...
    if (items.length === 0) {
        listEl.innerHTML = `<p class="text-slate-500 py-2">No summary available yet.</p>`;
        if (chevronBox) chevronBox.classList.add('hidden');
        if (toggleBtn) toggleBtn.classList.remove('cursor-pointer');
        return;
    }

    // Default bounds state for > 4 items
    if (items.length > 4) {
        if (chevronBox) chevronBox.classList.remove('hidden');
        if (toggleBtn) toggleBtn.classList.add('cursor-pointer');
        if (wrapper) {
            wrapper.style.maxHeight = '190px';
            wrapper.classList.add('overflow-y-auto');
    ...
    listEl.innerHTML = items.map((item, index) => {
        ...
        const animationStyle = animate ? `style="animation-fill-mode: both; animation-delay: ${index * 40}ms"` : '';
        const animationClass = animate ? `animate-slide-up-fade` : '';

        return `<div class="flex justify-between items-center py-3 border-b border-slate-200 last:border-0 ${animationClass}" ${animationStyle}>
                    <span class="text-ios-label whitespace-normal leading-tight">${escapeHTML(displayName)}</span>
                    <span class="font-bold ${colorClass} ml-3 shrink-0">₹${Math.abs(netAmount).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</span>
                </div>`;
    }).join('');
}
...
function updateStatusUI(state) {
    ...
    setTimeout(() => {
        if (state === 'offline') {
            ...
            userStatusDisplay.innerHTML = `<span class="text-sm text-slate-900">Welcome, <strong>${escapeHTML(name)}</strong></span>`;
        }
...
const APP_VERSION = "2.9.2: Edge Scrollbars, Custom Green & Slide-Fade";
